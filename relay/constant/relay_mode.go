...
const (
	...
	// Async T2A (MiniMax)
	RelayModeT2AAsyncSubmit
	RelayModeT2AAsyncFetch
)
...
func Path2RelayMode(path string) int {
	...
	} else if strings.HasPrefix(path, "/v1/t2a_v2") {
		// minimax native T2A endpoint, treat as audio speech
		relayMode = RelayModeAudioSpeech
	} else if strings.HasPrefix(path, "/v1/t2a_async_v2") {
		// async T2A submit
		relayMode = RelayModeT2AAsyncSubmit
	} else if strings.HasPrefix(path, "/v1/query/t2a_async_query_v2") {
		// async T2A fetch
		relayMode = RelayModeT2AAsyncFetch
