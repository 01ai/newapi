...
func (a *Adaptor) ConvertAudioRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.AudioRequest) (io.Reader, error) {
    ...
    // If the incoming body is Minimaxi native T2A (model/text/voice_setting...),
    // try to parse and forward it directly.
    if request.Input == "" {
        var nativeReq MiniMaxTTSRequest
        if err := common.UnmarshalBodyReusable(c, &nativeReq); err == nil && nativeReq.Text != "" {
            jsonData, mErr := json.Marshal(nativeReq)
            if mErr != nil {
                return nil, fmt.Errorf("error marshalling native minimax request: %w", mErr)
            }
            if nativeReq.OutputFormat != "" {
                c.Set("response_format", nativeReq.OutputFormat)
            }
